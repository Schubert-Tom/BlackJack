...
class Distribution():
    ...
    def __init__(self, card_distribution:tuple, amount_of_Cards:int):
        ...
        self.prob_distribution = tuple([Fraction(count,self.amount_of_Cards) for count in card_distribution])
...
def EW(PlayerVal:Hand, DealerVal:Hand, distribution:Distribution, dealernextCardnoTen=False):
    ...
    # Stand
    stand = DealerWinProb(PlayerVal=PlayerVal,DealerVal=DealerVal,distribution=distribution,dealernextCardnoTen=dealernextCardnoTen)
    ...
    if PlayerVal.value < 21:
        ...
        for card_val,card_count in enumerate(distribution.card_distribution):
            ...
            EW_Hit += distribution.prob_distribution[card_val-2] * EW(PlayerVal = newPlayerVal, DealerVal = DealerVal ,distribution = new_distribution, dealernextCardnoTen = dealernextCardnoTen)[0]
        ...
        if len(PlayerVal.handCards) == 2:
            if PlayerVal.value < 21:
                ...
                for card_val,card_count in enumerate(distribution.card_distribution):
                    ...
                    if card_count == 0:
                        continue
                    newPlayerVal = Hand(PlayerVal.handCards + tuple([card_val]),afterSplit=True)
                    new_distribution = distribution.removeCard(card_val)

                    dealerwinprob = DealerWinProb(newPlayerVal, DealerVal ,new_distribution, dealernextCardnoTen=dealernextCardnoTen)
                    double[0] += distribution.prob_distribution[card_val-2] * dealerwinprob[0]
                    double[1] += distribution.prob_distribution[card_val-2] * dealerwinprob[1]
                    double[2] += distribution.prob_distribution[card_val-2] * dealerwinprob[2]
            ...
            if PlayerVal.value < 21:
                ...
                if PlayerVal.handCards[0] != 11 or HIT_ON_SPLIT_ACES == True:
                    for card_val,card_count in enumerate(distribution.card_distribution):
                        ...
                        if card_count == 0:
                            continue
                        newPlayerVal = Hand(tuple(PlayerVal.handCards[:1]) + tuple([card_val]),afterSplit=True)
                        new_distribution = distribution.removeCard(card_val)
                        EW_Split += 2*distribution.prob_distribution[card_val-2] * EW(PlayerVal = newPlayerVal, DealerVal = DealerVal ,distribution = new_distribution, dealernextCardnoTen = dealernextCardnoTen)[0]   
                else:
                    # Aces
                    ...
                    for card_val,card_count in enumerate(distribution.card_distribution):
                        ...
                        new_distribution = distribution.removeCard(card_val)
                        dealerwinprob = DealerWinProb(newPlayerVal, DealerVal ,new_distribution, dealernextCardnoTen=dealernextCardnoTen)
                        split[0] += distribution.prob_distribution[card_val-2] * dealerwinprob[0]
                        split[1] += distribution.prob_distribution[card_val-2] * dealerwinprob[1]
                        split[2] += distribution.prob_distribution[card_val-2] * dealerwinprob[2]
...
def DealerWinProb(PlayerVal, DealerVal:Hand, distribution:Distribution, dealernextCardnoTen=False):
    ...
    if dealernextCardnoTen == True:
        ...
        cards[8] = 0
        modified_distribution = Distribution(card_distribution=cards, amount_of_Cards=sum(cards))
    ...
    for card_val,card_count in enumerate(distribution.card_distribution):
        ...
        new_distribution = distribution.removeCard(card_val)
        dealerwinprob = DealerWinProb(PlayerVal, newDealerVal ,new_distribution, dealernextCardnoTen=False)
        if dealernextCardnoTen == True:
            probability[0] += modified_distribution.prob_distribution[card_val-2] * dealerwinprob[0]
            probability[1] += modified_distribution.prob_distribution[card_val-2] * dealerwinprob[1]
            probability[2] += modified_distribution.prob_distribution[card_val-2] * dealerwinprob[2]
        else:
            probability[0] += distribution.prob_distribution[card_val-2] * dealerwinprob[0]
            probability[1] += distribution.prob_distribution[card_val-2] * dealerwinprob[1]
            probability[2] += distribution.prob_distribution[card_val-2] * dealerwinprob[2]
    return probability
...
@timer
def test():
    basicdistr = [2]*4*DECKS + [3]*4*DECKS + [4]*4*DECKS + [5]*4*DECKS + [6]*4*DECKS  + [7]*4*DECKS  + [8]*4*DECKS  + [9]*4*DECKS  + [10]*16*DECKS + [11]*4*DECKS
    shuffle(basicdistr)
